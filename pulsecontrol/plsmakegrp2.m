function grpdef2 = plsmakegrp2(name, ctrl, pulseInds)
...
for i = 1:length(name)
    ...
    switch makeType
        case 'pls'
            if ~iscell(grpdef.varpar), grpdef.varpar = {grpdef.varpar}; end
            if ~iscell(grpdef.params), grpdef.params = {grpdef.params}; end
            if length(grpdef.pulses)>1
                if length(grpdef.varpar)==1
                    varpar = grpdef.varpar{1};
                    grpdef.varpar = cell(1,length(grpdef.pulses));
                    [grpdef.varpar{:}]= deal(varpar);
                end
                if length(grpdef.params)==1
                    params = grpdef.params{1};
                    grpdef.params = cell(1,length(grpdef.pulses));
                    [grpdef.params{:}] = deal(params);
                end
            end
            ...
            varparLengths = cell2mat(cellfun(@(x)size(x,1),grpdef.varpar,'UniformOutput',0)); %length of each varpar
            npars = max(1,sum(varparLengths));
            parInds = ones(1,varparLengths(1)); %this will be lookup table to find which varpar and params cell to use for each pulse.
            for j = 2:length(varparLengths)
                parInds = [parInds, j*ones(1,varparLengths(j))];
            end
            ...
            plsDef = grpdef.pulses(parInds(pulseInds));%
            ...
            for j = 1:length(pulseInds)
                ...
                currPar = parInds(pulseInds); 
                params = grpdef.params{currPar}; % transfer valid pulse dependent parameters. Interpretation of nan may not be so useful here, but should not hurt.
                if ~isempty(grpdef.varpar) % assign varpar for this group
                    paramMask = ~isnan(grpdef.varpar{currPar}(pulseNum, :));
                    nVars = size(grpdef.varpar{currPar},2);
                    params(end-nVars + find(paramMask)) = grpdef.varpar{currPar}(pulseNum, paramMask);
                end

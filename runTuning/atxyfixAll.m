...
%   side: a side name, empty to do both. 
...
%   opt: amp: let you do it away from measurement point = 0. 
%   cent: start offset by vstep/2 
...
if isopt(opts,'chrg')
    ...
    for s=1:length(sides)
        ...
        for i=0:length(chans)
            ...
            if any(isnan(d{i+1}(:))); smset(1:19, gtvals); return; end
        end
    end
    if isopt(opts,'cent')
        for i =1:length(chans)

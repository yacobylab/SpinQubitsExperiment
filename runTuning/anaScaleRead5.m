function [fidelity,tAdj,STdiff,t1] = anaScaleRead5(opts,scan,data,dropTime,bins)
...
% drop_time set to 2.4 us if not given. This is 2 us for manipulation. 0.09 waiting at meas before marker sent, then 310 ns while tank circuit rings up.  
% ctrls: full: fit all timesteps (slow).
...
if exist('ctrl','var')&& isopt(opts,'fid')
    figure(91); clf; hold on;
    inds=sub2ind(size(fidArr),VtInd,1:length(VtInd));
    plot(1e6*tAdj,1-SfidFit(inds),'.-')    
    plot(1e6*tAdj,1-TfidFit(inds),'.-')

classdef Zoom < autotune.Op
    ...
    methods
        ...
        function out=ana(this,opts,data)
            % opts: noset(don't set the the measurement point), setOffset (center zoom along junc), man (choose zoom point manually)
            global tuneData
            if ~exist('opts','var'), opts = ''; end
            runNumber = tuneData.runNumber; 
            if ~exist('data','var') || isempty(data) || ischar(data) || numel(data)==1 % Check if loading old data or analyzing new scan. 
            ...
            plot(tuneData.chrg.trTriple(end,1),tuneData.chrg.trTriple(end,2),'r.','MarkerSize',10)
            plot(tuneData.chrg.blTriple(end,1),tuneData.chrg.blTriple(end,2),'r.','MarkerSize',10)
